...
(async function () {
  ...
  function renderSession(id) {
    ...
    const restart = (msg) => {
      ...
      app.replaceChildren(h('div', { class: 'intro' }, h('h1', null, 'Let’s start again'), h('p', { class: 'lead' }, msg), h('div', { class: 'actions' }, h('button', { class: 'btn primary', onclick: () => { location.hash = ''; route(); } }, 'Go to the start'))));
    };
    if (!sess) return restart('This run is not available in this browser, or the link was copied from another device.');
    // runs saved by an earlier version of the page: normalise or discard
    if (!sess.studies) { sess.studies = [sess.study]; for (const sl of sess.slates || []) if (!sl.study) sl.study = sess.study; }
    if (!Array.isArray(sess.slates) || !sess.slates.length || sess.slates.some((sl) => !DATA.studies[sl.study] || !sl.left || !sl.right)) return restart('This run was created with an older version of the study, so it cannot continue. Your previous answers are safe.');
    const ids = sess.studies;
